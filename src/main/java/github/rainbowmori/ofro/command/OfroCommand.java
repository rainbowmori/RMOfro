...
public class OfroCommand extends CommandTree {
  ...
  public OfroCommand() {
    ...
    then(new LiteralArgument("rtp")
        ...
        .then(OriginalArguments.onlinePlayer("player")
            .executes((commandSender, objects) -> {
            ...
            .then(OriginalArguments.worlds("worlds")
                .then(new BooleanArgument("bedInclude")
                    .executes((commandSender, objects) -> {
                      new RTP(((Player) objects.get(0)), commandSender)
                          .rtp((World) objects.get(1), ((boolean) objects.get(2)));
                    }))
                .executes((commandSender, objects) -> {
                  new RTP(((Player) objects.get(0)), commandSender)
    ...
    then(new LiteralArgument("savage")
        .then(OfroArguments.getSavageWorld("savageWorld")
            .then(new LiteralArgument("bed")
                    ...
                    .executes((commandSender, objects) -> {
                      ...
                      if (!savageWorld.isBedWorks()) {
                        throw CommandAPI.failWithString(
                                Util.cc("&c" + savageWorld.getName() + "ではベッドを使えません"));
                      }
                              ...
                              .ifPresent(location -> {
                                OfroPrefix.TP.send(commandSender,
                                    "<blue>%s <reset>: x = %s , y = %s ,z = %s".
                                        formatted(Bukkit.getOfflinePlayer(uuid).getName(), location.getX(),
