...
import github.rainbowmori.rainbowapi.dependencies.anvilgui.AnvilGUI.ResponseAction;
...
import github.rainbowmori.rainbowapi.dependencies.ui.menu.ItemInputMenu;
...
public class MaterialShop {
  ...
  public static boolean containMaterial(Material material) {
    return AllMaterial.containsKey(material);
  }

  public static MaterialInfo getInfo(Material material) {
    return AllMaterial.get(material);
  }

  private static MenuHolder<Ofro> shop = null;

  public static void openShop(Player player) {
    if (shop == null) {
      shop = new MenuHolder<>(Ofro.getPlugin(), 54, "<red>SHOP") {{
  ...
  private static final Map<Integer, ShopCategory> category = new HashMap<>() {{
    ...
    put(45, new ShopCategory(new RedirectItemButton<>(
        ...
        () -> new ItemInputMenu<>(Ofro.getPlugin(), "<aqua>ここに検索したいアイテムを入れてください",
            (itemStack, event) -> {
              ...
              event.getWhoClicked().openInventory(
                  new ItemValueShopPage(Ofro.getPlugin(),
    ...
    put(46, new ShopCategory(new ItemButton<>(
      ...
      public void onClick(MenuHolder<?> holder, InventoryClickEvent event) {
        GUIs.of(Ofro.getPlugin()).getStringInput("&b文字で検索", (s, completion) -> List.of(
                ResponseAction.openInventory(
                    new ItemValueShopPage(Ofro.getPlugin(), Util.mm("<red>" + s),
