...
public class OfroAlertItem extends CustomItem implements CoolityItem {
  ...
  public void rightClick(PlayerInteractEvent e) {
    ...
    if (CustomItem.itemCache.computeGetCache(uuid, new SavageItems(uuid)).isSonerUsed) {
      Ofro.getPlugin().getPrefixUtil().send(player, "<red>OfroSonerと同時使用はできません");
      return;
    }
    if (hasCooldown(uuid)) {
      Ofro.getPlugin().getPrefixUtil().send(player, "<red>すでにOfroAlertを使用しています");
      return;
    }
    if (useDurability(1)) {
      itemUse();
